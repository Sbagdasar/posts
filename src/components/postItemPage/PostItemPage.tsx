...
export const PostItemPage = () => {
    ...
    const [isLoadingComments, errorComments, isFetchingComments] = useFetching((id: number, comments: string) => {
    ...
    useEffect(() => {
        ...
        if (params.comments && params.id)
            isFetchingComments(+params.id, params.comments)
    }, [params])
    return (
        <div>
            {error && <h2>{error}</h2>}
            ...
            {
                isLoadingComments ? <Spin tip="Loading..." size="large" style={{
                        display: 'flex',
                        flexDirection: 'column',
                        justifyContent: 'center',
                        alignItems: 'center',
